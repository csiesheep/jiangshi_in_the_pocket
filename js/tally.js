...
export function houseLine() {
  ...
  if (!taken) {
    return left === 1
      ? "You have walked out of this house once. It has never kept you."
      : `You have walked out of this house ${times(left)}. It has never kept you.`;
  }
  const kept = `The house has taken you ${times(taken)}.`;
  if (!left) return kept;
  return `${kept} You have walked out ${times(left)}.`;
}
